...
public class FraudDetectionServiceV2 implements FraudDetectionService {
    ...
    @Value("${ml.v2.base-url:http://ml-fraud-service-2:5001}")
    ...
    public FraudDetectionDTO analyzeFraudRisk(FraudPredictionRequest req) {
        String url = baseUrl + endpoint; // ex: http://ml-fraud-service-2:5001/predict
        try {
            ...
            log.warn("ML V2 non-200: {}", resp.getStatusCode());
        } catch (Exception ex) {
            log.warn("ML V2 call failed: {}", ex.toString());
        }
        return null;
    }
}

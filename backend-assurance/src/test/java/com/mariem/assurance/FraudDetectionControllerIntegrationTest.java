 ...
 * Ces tests valident l'intégration REST -> services métier.
 */
@AutoConfigureMockMvc // (⚠️ filtres de sécurité actifs)
...
public class FraudDetectionControllerIntegrationTest {
    ...
    // === Config sécurité de test : JwtDecoder factice ===
    @TestConfiguration
    ...
    void testPredictEndpoint_FraudulentContract_ShouldDetectFraudAndTriggerAlert() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/fraud/predict")
                ...
                .andExpect(jsonPath("$.metadata.alertTriggered").doesNotExist());
    }
    ...
    void testStatisticsEndpoint_ShouldReturnStatistics() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/fraud/statistics"))
                ...
                .andExpect(jsonPath("$.detectionInfo.multiModelLogic")
                        .value("Active - Both models must detect fraud"))
    ...
    void testPredictEndpoint_InvalidData_ShouldReturnBadRequest() throws Exception {
        ...
        FraudPredictionRequest invalidRequest =
                new FraudPredictionRequest(invalidContract, invalidClient);
    ...
    // --- Helpers pour fabriquer des réponses ---
